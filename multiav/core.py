    ...
    def scan(self, path):
        try:
          ...
          '''
                ...
                scan_cmd = "docker exec {0} {2} --timeout {3} {1}".format(self.container.id, filename, self.binary_path,
                                                                          self.scan_timeout)
    ...
    def scan(self, path, max_speed=AV_SPEED.ALL, allow_internet=False, event_handlers=dict()):
        ...
        for engine_class in self.engines:
            ...
            if max_speed is None or engine.speed.value <= max_speed.value:
                ...
                print("[{0}] Skipping scan. Too slow! AV: {1} Max: {2}".format(engine.name, engine.speed.value,
                                                                               max_speed.value))
                continue
